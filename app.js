...
(function checkAuth() {
    ...
    if (!isLoggedIn && !localStorage.getItem('token') && !window.location.href.includes('login.html')) {
...
async function initApp() {
    try {
        ...
        script.src = `https://maps.googleapis.com/maps/api/js?key=${config.mapsApiKey || config.Maps_API_KEY}&libraries=places&callback=setupGoogleServices`;
        script.async = true; 
...
function setupGoogleServices() {
    ...
    drGo = new google.maps.DirectionsRenderer({ 
        polylineOptions: { strokeColor: "#18c979", strokeWeight: 6, strokeOpacity: 0.9 } 
    });
    drBack = new google.maps.DirectionsRenderer({ 
        polylineOptions: { strokeColor: "#222831", strokeWeight: 4, strokeOpacity: 0.4 }, 
...
async function calculate() {
    ...
    if (!googleMap) {
        googleMap = new google.maps.Map(mapEl, { 
            ...
            styles: [
                ...
                { "elementType": "labels.text.fill", "stylers": [{ "color": "#9aa3a0" }] },
    ...
    updateTripCard(locs, totalKm, totalToll, totalSec);
...
function updateTripCard(locs, km, toll, sec) {
    document.body.classList.add('trip-mode');

    const pickup = locs[0] || '-';
    const dropoff = locs[locs.length - 1] || '-';
    ...
    if (pickupText) pickupText.innerText = pickup;
    if (dropoffText) dropoffText.innerText = dropoff;
    if (routeSummary) routeSummary.innerText = `${km.toFixed(1)} km • ${Math.round(sec / 60)} min`;
    if (arrivalText) arrivalText.innerText = `Estimated trip cost is ready`;
    if (pinLabel) pinLabel.innerText = dropoff;
}
...
async function shareTrip() {
    const total = document.getElementById('total')?.innerText || '0.0';
    const km = document.getElementById('km')?.innerText || '0.0 km';
    const text = `LTravelLog estimate: HK$ ${total}, ${km}`;

    if (navigator.share) {
        try {
            await navigator.share({ title: 'LTravelLog Trip', text });
        } catch {}
    } else {
